        ...
        it('should GET an error message because of no tag', function(done) {
            chai.request(uri)
                ...
                .end((err, res) => {
                    ...
                    res.body.should.be.eql({ "error": "Tags parameter is required" });
                    done();
                });
        });
    });
    ...
    describe('Fetch with an incorrect \'direction\' query string', function() {
        it('should GET an error message', function(done) {
            chai.request(uri)
                ...
                .end((err, res) => {
                    res.should.have.status(400);
                    res.body.should.be.eql({ "error": "direction parameter is invalid" });
                    done();
                });
        });
    });
    ...
    describe('Fetch with an incorrect \'sortBy\' query string', function() {
        it('should GET an error message', function(done) {
            chai.request(uri)
                ...
                .end((err, res) => {
                    res.should.have.status(400);
                    res.body.should.be.eql({ "error": "sortBy parameter is invalid" });
                    done();
                });
        });
    });
